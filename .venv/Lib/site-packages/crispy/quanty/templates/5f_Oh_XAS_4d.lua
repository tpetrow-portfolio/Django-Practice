...
local T = {Tx_4d_5f, Ty_4d_5f, Tz_4d_5f}
Tv_4d_5f = CalculateT(T, Ev)
Th_4d_5f = CalculateT(T, Eh)
Tr_4d_5f = CalculateT(T, Er)
Tl_4d_5f = CalculateT(T, El)
Tk_4d_5f = CalculateT(T, WaveVector)

-- Initialize a table with the available spectra and the required operators.
SpectraAndOperators = {
    ["Isotropic Absorption"] = {Tk_4d_5f, Tr_4d_5f, Tl_4d_5f},
    ["Absorption"] = {Tk_4d_5f,},
    ["Circular Dichroic"] = {Tr_4d_5f, Tl_4d_5f},
    ["Linear Dichroic"] = {Tv_4d_5f, Th_4d_5f},
}
...
local T = {}
for Operator, _ in pairs(T_4d_5f) do
...
if CalculationRestrictions == nil then
    G_4d_5f = CreateSpectra(H_f, T_4d_5f, Psis_i, {{"Emin", Emin}, {"Emax", Emax}, {"NE", NPoints}, {"Gamma", Gamma}, {"DenseBorder", DenseBorder}})
else
    G_4d_5f = CreateSpectra(H_f, T_4d_5f, Psis_i, {{"Emin", Emin}, {"Emax", Emax}, {"NE", NPoints}, {"Gamma", Gamma}, {"Restrictions", CalculationRestrictions}, {"DenseBorder", DenseBorder}})
end

if ShiftSpectra then
    G_4d_5f.Shift(ZeroShift + ExperimentalShift)
end

-- Create a list with the Boltzmann probabilities for a given operator and wavefunction.
local dZ_4d_5f = {}
for _ in ipairs(T_4d_5f) do
    for j in ipairs(Psis_i) do
        table.insert(dZ_4d_5f, dZ_i[j])
    end
end

local Ids = {}
for k, v in pairs(T_4d_5f) do
    Ids[v] = k
end
...
for Spectrum, Operators in pairs(SpectraAndOperators) do
    if ValueInTable(Spectrum, SpectraToCalculate) then
        ...
        if Spectrum == "Isotropic Absorption" then
            Giso = GetSpectrum(G_4d_5f, SpectrumIds, dZ_4d_5f, #T_4d_5f, #Psis_i)
            Giso = Giso / 3
            SaveSpectrum(Giso, Prefix .. "_iso", Gaussian, Lorentzian, Pcl_4d_5f)
        end

        if Spectrum == "Absorption" then
            Gk = GetSpectrum(G_4d_5f, SpectrumIds, dZ_4d_5f, #T_4d_5f, #Psis_i)
            SaveSpectrum(Gk, Prefix .. "_k", Gaussian, Lorentzian, Pcl_4d_5f)
        end

        if Spectrum == "Circular Dichroic" then
            Gr = GetSpectrum(G_4d_5f, SpectrumIds[1], dZ_4d_5f, #T_4d_5f, #Psis_i)
            Gl = GetSpectrum(G_4d_5f, SpectrumIds[2], dZ_4d_5f, #T_4d_5f, #Psis_i)
            SaveSpectrum(Gr, Prefix .. "_r", Gaussian, Lorentzian, Pcl_4d_5f)
            SaveSpectrum(Gl, Prefix .. "_l", Gaussian, Lorentzian, Pcl_4d_5f)
            SaveSpectrum(Gr - Gl, Prefix .. "_cd", Gaussian, Lorentzian)
        end

        if Spectrum == "Linear Dichroic" then
            Gv = GetSpectrum(G_4d_5f, SpectrumIds[1], dZ_4d_5f, #T_4d_5f, #Psis_i)
            Gh = GetSpectrum(G_4d_5f, SpectrumIds[2], dZ_4d_5f, #T_4d_5f, #Psis_i)
            SaveSpectrum(Gv, Prefix .. "_v", Gaussian, Lorentzian, Pcl_4d_5f)
            SaveSpectrum(Gh, Prefix .. "_h", Gaussian, Lorentzian, Pcl_4d_5f)
