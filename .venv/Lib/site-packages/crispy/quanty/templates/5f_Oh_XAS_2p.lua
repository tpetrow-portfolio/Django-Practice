...
if AtomicTerm then
    ...
    F2_2p_5f_f = $F2(2p,5f)_f_value * $F2(2p,5f)_f_scaleFactor
    G2_2p_5f_f = $G2(2p,5f)_f_value * $G2(2p,5f)_f_scaleFactor
    G4_2p_5f_f = $G4(2p,5f)_f_value * $G4(2p,5f)_f_scaleFactor
...
local T = {Txy_2p_5f, Txz_2p_5f, Tyz_2p_5f, Tx2y2_2p_5f, Tz2_2p_5f}
Tv_2p_5f = CalculateT(T, Ev, WaveVector)
Th_2p_5f = CalculateT(T, Eh, WaveVector)
Tr_2p_5f = CalculateT(T, Er, WaveVector)
Tl_2p_5f = CalculateT(T, El, WaveVector)
...
local T = {}
for Operator, _ in pairs(T_2p_5f) do
...
if CalculationRestrictions == nil then
    G_2p_5f = CreateSpectra(H_f, T_2p_5f, Psis_i, {{"Emin", Emin}, {"Emax", Emax}, {"NE", NPoints}, {"Gamma", Gamma}, {"DenseBorder", DenseBorder}})
else
    G_2p_5f = CreateSpectra(H_f, T_2p_5f, Psis_i, {{"Emin", Emin}, {"Emax", Emax}, {"NE", NPoints}, {"Gamma", Gamma}, {"Restrictions", CalculationRestrictions}, {"DenseBorder", DenseBorder}})
end

if ShiftSpectra then
    G_2p_5f.Shift(ZeroShift + ExperimentalShift)
end

-- Create a list with the Boltzmann probabilities for a given operator and wavefunction.
local dZ_2p_5f = {}
for _ in ipairs(T_2p_5f) do
    for j in ipairs(Psis_i) do
        table.insert(dZ_2p_5f, dZ_i[j])
    end
end

local Ids = {}
for k, v in pairs(T_2p_5f) do
    Ids[v] = k
end
...
for Spectrum, Operators in pairs(SpectraAndOperators) do
    if ValueInTable(Spectrum, SpectraToCalculate) then
        ...
        if Spectrum == "Isotropic Absorption" then
            Giso = GetSpectrum(G_2p_5f, SpectrumIds, dZ_2p_5f, #T_2p_5f, #Psis_i)
            Giso = Giso / 15
            SaveSpectrum(Giso, Prefix .. "_iso", Gaussian, Lorentzian, Pcl_2p_5f)
        end
        ...
        if Spectrum == "Circular Dichroic" then
            Gr = GetSpectrum(G_2p_5f, SpectrumIds[1], dZ_2p_5f, #T_2p_5f, #Psis_i)
            Gl = GetSpectrum(G_2p_5f, SpectrumIds[2], dZ_2p_5f, #T_2p_5f, #Psis_i)
            SaveSpectrum(Gr, Prefix .. "_r", Gaussian, Lorentzian, Pcl_2p_5f)
            SaveSpectrum(Gl, Prefix .. "_l", Gaussian, Lorentzian, Pcl_2p_5f)
            SaveSpectrum(Gr - Gl, Prefix .. "_cd", Gaussian, Lorentzian)
        end

        if Spectrum == "Linear Dichroic" then
            Gv = GetSpectrum(G_2p_5f, SpectrumIds[1], dZ_2p_5f, #T_2p_5f, #Psis_i)
            Gh = GetSpectrum(G_2p_5f, SpectrumIds[2], dZ_2p_5f, #T_2p_5f, #Psis_i)
            SaveSpectrum(Gv, Prefix .. "_v", Gaussian, Lorentzian, Pcl_2p_5f)
            SaveSpectrum(Gh, Prefix .. "_h", Gaussian, Lorentzian, Pcl_2p_5f)
