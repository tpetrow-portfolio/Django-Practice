...
if AtomicTerm then
    ...
    F2_2p_4d_f = $F2(2p,4d)_f_value * $F2(2p,4d)_f_scaleFactor
    G1_2p_4d_f = $G1(2p,4d)_f_value * $G1(2p,4d)_f_scaleFactor
    G3_2p_4d_f = $G3(2p,4d)_f_value * $G3(2p,4d)_f_scaleFactor
...
if LmctLigandsHybridizationTerm then
    ...
    E_4d_f = (10 * Delta_4d_L1_f - NElectrons_4d * (31 + NElectrons_4d) * U_4d_4d_f / 2 - 90 * U_2p_4d_f) / (16 + NElectrons_4d)
    E_2p_f = (10 * Delta_4d_L1_f + (1 + NElectrons_4d) * (NElectrons_4d * U_4d_4d_f / 2 - (10 + NElectrons_4d) * U_2p_4d_f)) / (16 + NElectrons_4d)
    E_L1_f = ((1 + NElectrons_4d) * (NElectrons_4d * U_4d_4d_f / 2 + 6 * U_2p_4d_f) - (6 + NElectrons_4d) * Delta_4d_L1_f) / (16 + NElectrons_4d)
...
SpectraAndOperators = {
    ["Isotropic Absorption"] = {Tk_2p_4d, Tr_2p_4d, Tl_2p_4d},
    ["Absorption"] = {Tk_2p_4d,},
    ["Circular Dichroic"] = {Tr_2p_4d, Tl_2p_4d},
    ["Linear Dichroic"] = {Tv_2p_4d, Th_2p_4d},
}
...
local T = {}
for Operator, _ in pairs(T_2p_4d) do
...
if CalculationRestrictions == nil then
    G_2p_4d = CreateSpectra(H_f, T_2p_4d, Psis_i, {{"Emin", Emin}, {"Emax", Emax}, {"NE", NPoints}, {"Gamma", Gamma}, {"DenseBorder", DenseBorder}})
else
    G_2p_4d = CreateSpectra(H_f, T_2p_4d, Psis_i, {{"Emin", Emin}, {"Emax", Emax}, {"NE", NPoints}, {"Gamma", Gamma}, {"Restrictions", CalculationRestrictions}, {"DenseBorder", DenseBorder}})
end
...
local dZ_2p_4d = {}
for _ in ipairs(T_2p_4d) do
    for j in ipairs(Psis_i) do
        table.insert(dZ_2p_4d, dZ_i[j])
    end
end

local Ids = {}
for k, v in pairs(T_2p_4d) do
    Ids[v] = k
end
...
for Spectrum, Operators in pairs(SpectraAndOperators) do
    if ValueInTable(Spectrum, SpectraToCalculate) then
        ...
        if Spectrum == "Isotropic Absorption" then
            Giso = GetSpectrum(G_2p_4d, SpectrumIds, dZ_2p_4d, #T_2p_4d, #Psis_i)
            Giso = Giso / 3
            SaveSpectrum(Giso, Prefix .. "_iso", Gaussian, Lorentzian, Pcl_2p_4d)
        end

        if Spectrum == "Absorption" then
            Gk = GetSpectrum(G_2p_4d, SpectrumIds, dZ_2p_4d, #T_2p_4d, #Psis_i)
            SaveSpectrum(Gk, Prefix .. "_k", Gaussian, Lorentzian, Pcl_2p_4d)
        end

        if Spectrum == "Circular Dichroic" then
            Gr = GetSpectrum(G_2p_4d, SpectrumIds[1], dZ_2p_4d, #T_2p_4d, #Psis_i)
            Gl = GetSpectrum(G_2p_4d, SpectrumIds[2], dZ_2p_4d, #T_2p_4d, #Psis_i)
            SaveSpectrum(Gr, Prefix .. "_r", Gaussian, Lorentzian, Pcl_2p_4d)
            SaveSpectrum(Gl, Prefix .. "_l", Gaussian, Lorentzian, Pcl_2p_4d)
            SaveSpectrum(Gr - Gl, Prefix .. "_cd", Gaussian, Lorentzian)
        end

        if Spectrum == "Linear Dichroic" then
            Gv = GetSpectrum(G_2p_4d, SpectrumIds[1], dZ_2p_4d, #T_2p_4d, #Psis_i)
            Gh = GetSpectrum(G_2p_4d, SpectrumIds[2], dZ_2p_4d, #T_2p_4d, #Psis_i)
            SaveSpectrum(Gv, Prefix .. "_v", Gaussian, Lorentzian, Pcl_2p_4d)
            SaveSpectrum(Gh, Prefix .. "_h", Gaussian, Lorentzian, Pcl_2p_4d)
