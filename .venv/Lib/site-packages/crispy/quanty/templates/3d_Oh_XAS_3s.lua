...
if AtomicTerm then
    ...
    G2_3s_3d_f = $G2(3s,3d)_f_value * $G2(3s,3d)_f_scaleFactor
...
if LmctLigandsHybridizationTerm then
    ...
    E_L1_f = (-2 * Delta_3d_L1_f * NElectrons_3d - 4 * Delta_3d_L1_f + U_3d_3d_f * NElectrons_3d^2 + U_3d_3d_f * NElectrons_3d + 4 * U_3s_3d_f * NElectrons_3d + 4 * U_3s_3d_f) / (2 * (NElectrons_3d + 12))
...
if MlctLigandsHybridizationTerm then
    ...
    E_L2_f = (2 * Delta_3d_L2_f * NElectrons_3d + 4 * Delta_3d_L2_f + U_3d_3d_f * NElectrons_3d^2 - U_3d_3d_f * NElectrons_3d - 4 * U_3d_3d_f + 4 * U_3s_3d_f * NElectrons_3d + 4 * U_3s_3d_f) / (2 * (NElectrons_3d + 2))
...
local T = {Txy_3s_3d, Txz_3s_3d, Tyz_3s_3d, Tx2y2_3s_3d, Tz2_3s_3d}
Tv_3s_3d = CalculateT(T, Ev, WaveVector)
Th_3s_3d = CalculateT(T, Eh, WaveVector)
Tr_3s_3d = CalculateT(T, Er, WaveVector)
Tl_3s_3d = CalculateT(T, El, WaveVector)
...
local T = {}
for Operator, _ in pairs(T_3s_3d) do
...
if CalculationRestrictions == nil then
    ...
    G_3s_3d = CreateSpectra(H_f, T_3s_3d, Psis_i, {{"Emin", Emin}, {"Emax", Emax}, {"NE", NPoints}, {"Gamma", Gamma}, {"Restrictions", CalculationRestrictions}, {"DenseBorder", DenseBorder}})
end

if ShiftSpectra then
    G_3s_3d.Shift(ZeroShift + ExperimentalShift)
end

-- Create a list with the Boltzmann probabilities for a given operator and wavefunction.
local dZ_3s_3d = {}
for _ in ipairs(T_3s_3d) do
    for j in ipairs(Psis_i) do
        table.insert(dZ_3s_3d, dZ_i[j])
    end
end

local Ids = {}
for k, v in pairs(T_3s_3d) do
    Ids[v] = k
end
...
for Spectrum, Operators in pairs(SpectraAndOperators) do
    if ValueInTable(Spectrum, SpectraToCalculate) then
        ...
        if Spectrum == "Isotropic Absorption" then
            Giso = GetSpectrum(G_3s_3d, SpectrumIds, dZ_3s_3d, #T_3s_3d, #Psis_i)
            Giso = Giso / 15
            SaveSpectrum(Giso, Prefix .. "_iso", Gaussian, Lorentzian, Pcl_3s_3d)
        end
        ...
        if Spectrum == "Circular Dichroic" then
            Gr = GetSpectrum(G_3s_3d, SpectrumIds[1], dZ_3s_3d, #T_3s_3d, #Psis_i)
            Gl = GetSpectrum(G_3s_3d, SpectrumIds[2], dZ_3s_3d, #T_3s_3d, #Psis_i)
            SaveSpectrum(Gr, Prefix .. "_r", Gaussian, Lorentzian, Pcl_3s_3d)
            SaveSpectrum(Gl, Prefix .. "_l", Gaussian, Lorentzian, Pcl_3s_3d)
            SaveSpectrum(Gr - Gl, Prefix .. "_cd", Gaussian, Lorentzian)
        end

        if Spectrum == "Linear Dichroic" then
            Gv = GetSpectrum(G_3s_3d, SpectrumIds[1], dZ_3s_3d, #T_3s_3d, #Psis_i)
            Gh = GetSpectrum(G_3s_3d, SpectrumIds[2], dZ_3s_3d, #T_3s_3d, #Psis_i)
            SaveSpectrum(Gv, Prefix .. "_v", Gaussian, Lorentzian, Pcl_3s_3d)
            SaveSpectrum(Gh, Prefix .. "_h", Gaussian, Lorentzian, Pcl_3s_3d)
